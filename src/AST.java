...
public class AST {
  ...
  /**
   * Replaces unnecessary non-leaf nodes by their children (ex : Instruction), and
   * removes a lot of unnecessary terminals
   * 
      ...
      switch (children.get(i).getLabel().getType()) {
        ...
        case PRODPRIME:
          if (children.get(i).getChildren().size() == 1
              && children.get(i).getChildren().get(0).getLabel().getType() == LexicalUnit.EPSILON) {
            children.remove(i);
            i--;
          } else {
        ...
        case PROD:
          ...
          if (children.get(i).getChildren().size() == 2
              && children.get(i).getChildren().get(1).getChildren().size() == 1) {
    ...
    while (i < children.size()) {
      switch (children.get(i).getLabel().getType()) {
        case ATOM:
          ...
          if (children.get(i).getChildren().size() == 2) {
            children.get(i).getChildren().get(1).getLabel()
                .setValue("-" + children.get(i).getChildren().get(1).getLabel().getValue());
        ...
        case PROD:
          if (children.get(i).getChildren().size() != 1) {
            children.set(i,
                setUpOperators(new ParseTree(
                    new Symbol(children.get(i).getChildren().get(1).getChildren().get(0).getLabel().getType(),
                        children.get(i).getChildren().get(1).getChildren().get(0).getLabel().getValue()),
                    children.get(i).getChildren())));
          } else {
        ...
        case PRODPRIME:
          if (children.get(i).getChildren().size() == 3) {
            children.set(i,
                setUpOperators(new ParseTree(
                    new Symbol(children.get(i).getChildren().get(2).getChildren().get(0).getLabel().getType(),
                        children.get(i).getChildren().get(2).getChildren().get(0).getLabel().getValue()),
                    children.get(i).getChildren())));
          } else {
            children.set(i, setUpOperators(children.get(i)));
          }
  ...
  private static ParseTree removeExprArith(ParseTree parseTree) {
    ...
    while (i < children.size()) {
      switch (children.get(i).getLabel().getType()) {
        ...
        case EXPRARITHPRIME:
          if (children.get(i).getChildren().size() == 1
              && (children.get(i).getChildren().get(0).getLabel().getType() == LexicalUnit.NUMBER
                  || children.get(i).getChildren().get(0).getLabel().getType() == LexicalUnit.VARNAME)) {
   ...
   * a ParseTree with left associativity respected. This function can be called
   * for additions and substractions, of for multiplications and divisions.
   * 
   * @param i        index of the child where the first operator concerned is
   *                 present in the original ParseTree
  ...
  private static ParseTree getChildParseTree(int i, List<ParseTree> children, LexicalUnit op1, LexicalUnit op2) {
    ...
    // Create a lost of all consecutive nodes corresponding to the operators
    // specified
